class NumberRequestsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @number_request.save
        format.html { redirect_to number_request_url(@number_request), notice: "Number request was successfully created." }
  ...
  def update
    respond_to do |format|
      if @number_request.update(number_request_params)
        format.html { redirect_to number_request_url(@number_request), notice: "Number request was successfully updated." }
  ...
  def number_request_params
    params.require(:number_request).permit(:input, :response)
  end

  def make_api_request(input)
    uri = URI("http://localhost:3000/api/v1/numbers/")
    ...
    if numbers.length == 1
      ...
      uri.path += "batch"
      response = Net::HTTP.post_form(uri, 'numbers[]' => numbers).body
    end
    response
  end
end
